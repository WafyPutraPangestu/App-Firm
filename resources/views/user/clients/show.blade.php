                            ...
                            <div class="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all">
                                <div class="flex justify-between items-start mb-4">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2">
                                            <h3 class="text-xl font-bold text-gray-800">{{ $perkara->judul_perkara }}</h3>
                                            <span class="px-3 py-1 rounded-full text-xs font-semibold
                                                @if($perkara->status == 'berjalan') bg-green-100 text-green-800
                                                @else bg-gray-100 text-gray-800
                                                @endif">
                                                {{ ucfirst($perkara->status) }}
                                            </span>
                                        </div>
                                        <p class="text-gray-600 mb-2">{{ $perkara->deskripsi }}</p>
                                ...
                                <div class="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
                                    <button @click="openProgressModal({{ $perkara->id }}, '{{ $perkara->judul_perkara }}')" 
                                            class="flex items-center px-4 py-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-all">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                                        </svg>
                                        Lihat Progress ({{ $perkara->progres->count() }})
                                    </button>

                                    <button @click="openInvoiceModal({{ $perkara->id }}, '{{ $perkara->judul_perkara }}')" 
                                            class="flex items-center px-4 py-2 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition-all">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/>
                                        </svg>
                                        Lihat Invoice ({{ $perkara->invoices->count() }})
                                    </button>

                                    <button @click="openDocumentModal({{ $perkara->id }}, '{{ $perkara->judul_perkara }}')" 
                                            class="flex items-center px-4 py-2 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-all">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                                        </svg>
                                        Lihat Dokumen ({{ $perkara->documents->count() }})
                                    </button>
                                </div>
                            </div>
                        @endforeach
    ...
    <script>
        function clientPortal() {
            return {
                ...
                // Di bagian JavaScript clientPortal()
